...
<head>
    ...
    <style>
        /* (Tetap gunakan semua style yang sudah kamu buat) */
        * {
...
<body class="bg-gray-50">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside
            ...
            <div class="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                ...
                <!-- Tombol Buat Forum (hanya teacher) -->
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                <div class="p-4 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="chat-item p-3 rounded-xl hover:bg-gray-50 cursor-pointer {{ request()->route('discussionForums.show', $forum->id) ? 'bg-blue-50 border-l-4 border-blue-500' : '' }}"
                            onclick="window.location='{{ route('discussionForums.show', $forum->id) }}'">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between mb-1">
                                        <h4 class="font-semibold text-gray-800 truncate">{{ $forum->title }}</h4>
                                        <span class="text-xs text-gray-500">
                                            {{ $forum->comments->count() > 0 ? $forum->comments->last()->created_at->format('H:i') : 'New' }}
                                        </span>
                                    </div>
                                    <p class="text-sm text-gray-600 truncate">
                                        {{ $forum->comments->last()?->comment ?? 'No messages yet' }}
                                    </p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span
                                            class="inline-flex items-center text-xs text-green-600 bg-green-100 px-2 py-0.5 rounded-full">
                                            <span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1"></span>
                                            {{ $forum->members->count() }} members
                                        </span>
                                    </div>
                                </div>
        ...
        <main class="flex-1 flex flex-col bg-white">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2 text-sm text-gray-600">
                                    ...
                                    <span class="flex items-center text-green-600">
                                        <span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1"></span>
                                        {{ $forum->members->where('user_id', '!=', Auth::id())->count() + 1 }} online
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar bg-gradient-to-b from-gray-50 to-white p-4">
                <div class="max-w-4xl mx-auto space-y-6">
                    ...
                    <div
                        ...
                        <div
                            class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-400 to-gray-600 flex items-center justify-center text-white text-sm font-medium">
                            {{ Str::upper(Str::substr($comment->user->name, 0, 2)) }}
                        </div>
                        @endif

                        <div class="{{ $comment->user_id === Auth::id() ? 'items-end' : '' }} flex-1">
                            <div
                        ...
                        <div
                            ...
                            {{ Str::upper(Str::limit(Auth::user()->name, 2)) }}
                        </div>
                        @endif
                    </div>
                    @endforeach
            ...
            @else
            <!-- Default view ketika belum pilih forum -->
            <div class="flex-1 flex items-center justify-center">
                <div class="text-center max-w-md">
                    <i data-lucide="message-square" class="w-24 h-24 text-gray-300 mx-auto mb-6"></i>
                    <h3 class="text-2xl font-medium mb-2">Welcome to Discussion Forums</h3>
    ...
    <script>
        ...
        // Klik chat item → highlight + mobile close
