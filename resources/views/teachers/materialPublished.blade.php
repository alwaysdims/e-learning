@extends('teachers.layouts.main',['title' => 'Material Published'])
...
<div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="alert alert-primary show mb-4">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <small>Mata Pelajaran: {{ $material->subject->name ?? '-' }}</small>
                    @if($material->content)
                    | <small>File: <a href="{{ Storage::url($material->content) }}" target="_blank"
                            class="text-white">{{ basename($material->content) }}</a></small>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto mt-3 sm:mt-0">
            <form action="{{ route('teacher.materials.published', $material->id) }}" method="GET">
        ...
        <table class="table table-report -mt-2">
            <thead>
                <tr>
                    ...
                    <th class="text-center whitespace-nowrap">DIPUBLISH PADA</th>
            ...
            <tbody>
                ...
                <tr class="intro-x">
                    ...
                    <td class="text-center">{{ $publish->classRoom->name }}</td>
                    <td class="text-center">
                        {{ $publish->description ?? '-' }}
                    </td>
                    <td class="text-center">
                        {{ \Carbon\Carbon::parse($publish->published_at)->format('D, d M Y | H:i A') }}
                    </td>
                    <td class="table-report__action">
                        <div class="flex justify-center items-center">
                            <!-- Edit Deskripsi -->
                            <a class="flex items-center mr-3 text-primary" href="javascript:;" data-tw-toggle="modal"
                                data-tw-target="#edit-modal-{{ $publish->id }}">
                                <i data-lucide="edit" class="w-4 h-4 mr-1"></i> Edit
                            </a>

                            <!-- Hapus Publish -->
                            <a class="flex items-center text-danger" href="javascript:;" data-tw-toggle="modal"
                                data-tw-target="#delete-confirmation-modal-{{ $publish->id }}">
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h2 class="font-medium text-base mr-auto">Edit Deskripsi Publish</h2>
                            </div>
                            <form action="{{ route('teacher.materials.published_update', $publish->id) }}"
                                method="POST">
                                @csrf @method('PUT')
                                <div class="modal-body">
                                    <label class="form-label">Deskripsi (opsional)</label>
                                    <textarea name="description" class="form-control"
                                        rows="4">{{ $publish->description }}</textarea>
                                    <small class="text-slate-500">Kelas: {{ $publish->classRoom->name }}</small>
                                </div>
                        ...
                        <div class="modal-content">
                            <div class="modal-body p-10 text-center">
                                ...
                                <div class="text-slate-500 mt-2">
                                    Materi "{{ $material->title }}" akan dihapus dari kelas
                                    "{{ $publish->classRoom->name }}"
                                </div>
                            </div>
                            <div class="modal-footer text-center">
                                <button type="button" data-tw-dismiss="modal"
                                    class="btn btn-outline-secondary w-24 mr-3">Cancel</button>
                                <form action="{{ route('teacher.materials.published_destroy', $publish->id) }}"
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center py-8">Materi belum dipublish ke kelas manapun.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="insert-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Publish Materi ke Kelas</h2>
            </div>
            <form action="{{ route('teacher.materials.published_store', $material->id) }}" method="POST">
                @csrf
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    ...
                    <div class="col-span-12">
                        <label class="form-label">Deskripsi / Catatan untuk Siswa (opsional)</label>
                        <textarea name="description" class="form-control" rows="4"
                            placeholder="Contoh: Silakan baca materi ini sebelum ujian minggu depan..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" data-tw-dismiss="modal"
