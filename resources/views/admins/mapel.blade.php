@extends('admins.layouts.main',['title' => 'Subjects'])
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <div class="text-center">
            <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#insert-modal"
                class="btn btn-primary shadow-md mr-2">Add Subject</a>
        </div>
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="text-center whitespace-nowrap">SUBJECT NAME</th>
                ...
                <tbody>
                    @forelse($subjects as $index => $subject)
                        <tr class="intro-x">
                            <td class="text-center">{{ $subjects->firstItem() + $index }}</td>
                            <td class="text-center font-medium">{{ $subject->name }}</td>
                            <td class="table-report__action">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3 text-primary" href="javascript:;"
                                        data-tw-toggle="modal" data-tw-target="#edit-modal-{{ $subject->id }}">
                                    ...
                                    <a class="flex items-center text-danger" href="javascript:;"
                                        data-tw-toggle="modal" data-tw-target="#delete-confirmation-modal-{{ $subject->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        ...
                        <!-- Edit Modal -->
                        <div id="edit-modal-{{ $subject->id }}" class="modal" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h2 class="font-medium text-base mr-auto">Update Subject</h2>
                                    </div>
                        ...
                        <div id="delete-confirmation-modal-{{ $subject->id }}" class="modal" tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body p-10 text-center">
                                        ...
                                        <div class="text-slate-500 mt-2">Delete subject "{{ $subject->name }}"?</div>
                                    </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-8">No subject found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- Insert Modal (Add Subject) -->
<div id="insert-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Add Subject</h2>
            </div>
            <form action="{{ route('admin.subjects.store') }}" method="POST">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12">
                        <label class="form-label">Subject Name</label>
                        <input type="text" name="name" class="form-control" placeholder="e.g. Matematika" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-20 mr-1">Cancel</button>
