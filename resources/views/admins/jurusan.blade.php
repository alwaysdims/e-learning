@extends('admins.layouts.main',['title' => 'Majors'])
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <div class="text-center">
            <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#insert-modal"
                class="btn btn-primary shadow-md mr-2">Add Major</a>
        </div>
        <div class="hidden md:block mx-auto text-slate-500"></div>
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <form action="{{ route('admin.majors.index') }}" method="GET">
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="text-center whitespace-nowrap">CODE MAJOR</th>
                        <th class="text-center whitespace-nowrap">MAJOR NAME</th>
                ...
                <tbody>
                    @forelse($majors as $index => $major)
                        <tr class="intro-x">
                            <td class="text-center">{{ $majors->firstItem() + $index }}</td>
                            <td class="text-center">{{ $major->code_major }}</td>
                            <td class="text-center font-medium">{{ $major->name }}</td>
                            <td class="table-report__action">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3 text-primary" href="javascript:;"
                                        data-tw-toggle="modal" data-tw-target="#edit-modal-{{ $major->id }}">
                                    ...
                                    <a class="flex items-center text-danger" href="javascript:;"
                                        data-tw-toggle="modal" data-tw-target="#delete-confirmation-modal-{{ $major->id }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        ...
                        <!-- Edit Modal -->
                        <div id="edit-modal-{{ $major->id }}" class="modal" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h2 class="font-medium text-base mr-auto">Update Major</h2>
                                    </div>
                                    <form action="{{ route('admin.majors.update', $major->id) }}" method="POST">
                                        @csrf @method('PUT')
                                        <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                                            <div class="col-span-12">
                                                <label class="form-label">Code Major</label>
                                                <input type="text" name="code_major" class="form-control"
                                                    value="{{ $major->code_major }}" required>
                                            </div>
                                            <div class="col-span-12">
                        ...
                        <div id="delete-confirmation-modal-{{ $major->id }}" class="modal" tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body p-10 text-center">
                                        ...
                                        <div class="text-slate-500 mt-2">Delete major "{{ $major->name }}"?</div>
                                    </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-8">No major found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- Insert Modal (Add Major) -->
<div id="insert-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Add Major</h2>
            </div>
            <form action="{{ route('admin.majors.store') }}" method="POST">
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12">
                        <label class="form-label">Code Major</label>
                        <input type="text" name="code_major" class="form-control" placeholder="e.g. RPL" required>
                    </div>
                    <div class="col-span-12">
                        <label class="form-label">Major Name</label>
                        <input type="text" name="name" class="form-control" placeholder="e.g. Rekayasa Perangkat Lunak" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-20 mr-1">Cancel</button>
