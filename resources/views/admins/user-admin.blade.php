@extends('admins.layouts.main',['title' => 'Admins'])
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <div class="text-center">
            <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#insert-modal"
                class="btn btn-primary shadow-md mr-2">Add Admin</a>
        </div>
        <div class="hidden md:block mx-auto text-slate-500"></div>
        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
            <form action="{{ route('admin.user.admin.index') }}" method="GET">
            ...
            <tbody>
                @forelse($admins as $index => $admin)
                    <tr class="intro-x">
                        ...
                        <td class="text-center">{{ $admin->admin?->address ?? '-' }}</td>
                        <td class="text-center">{{ $admin->admin?->no_telp ?? '-' }}</td>
                        <td class="table-report__action">
                            <div class="flex justify-center items-center">
                                <a class="flex items-center mr-3 text-primary" href="javascript:;"
                                    data-tw-toggle="modal" data-tw-target="#edit-modal-{{ $admin->id }}">
                                ...
                                <a class="flex items-center text-danger" href="javascript:;"
                                    data-tw-toggle="modal" data-tw-target="#delete-confirmation-modal-{{ $admin->id }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <!-- Edit Modal (unique per admin) -->
                    <div id="edit-modal-{{ $admin->id }}" class="modal" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h2 class="font-medium text-base mr-auto">Update Admin</h2>
                                </div>
                    ...
                    <div id="delete-confirmation-modal-{{ $admin->id }}" class="modal" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-body p-10 text-center">
                                    ...
                                    <div class="text-slate-500 mt-2">Delete admin "{{ $admin->name }}"?</div>
                                </div>
                                <div class="modal-footer text-center">
                                    <button type="button" data-tw-dismiss="modal"
                                        class="btn btn-outline-secondary w-24 mr-3">Cancel</button>
                                    <form action="{{ route('admin.user.admin.destroy', $admin->id) }}"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-8">No admin found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<!-- Insert Modal (Add Admin) -->
<div id="insert-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Add Admin</h2>
            </div>
